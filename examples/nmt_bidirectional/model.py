from tensorflow.python.keras.layers import Input, GRU, Dense, Concatenate, TimeDistributed, Bidirectional, CuDNNLSTM, CuDNNGRU
...
def define_nmt(hidden_size, batch_size, en_timesteps, en_vsize, fr_timesteps, fr_vsize):
    ...
    # Encoder GRU
    encoder_gru = Bidirectional(RECURRENT(hidden_size, return_sequences=True, return_state=True, name='encoder_gru'), name='bidirectional_encoder')
    print(encoder_gru(encoder_inputs))
    encoder_out, encoder_fwd_state, encoder_back_state = encoder_gru(encoder_inputs)

    # Set up the decoder GRU, using `encoder_states` as initial state.
    decoder_gru = RECURRENT(hidden_size*2, return_sequences=True, return_state=True, name='decoder_gru')
